...
sys.path.append(os.path.abspath('./Networks/dgcnn/utils'))
sys.path.append(os.path.abspath('./Networks/dgcnn/models'))

import tf_util
from transform_nets import input_transform_net

def get_model(point_cloud, seg_label, input_label, is_training, cat_num, part_num, \
    ...
    #test
    if test_fl:
        nn_idx = same_label_topology(point_cloud,seg_label, k)
    ...
    #test
    if test_fl:
        nn_idx = same_label_topology(point_cloud_transformed,seg_label, k)
    ...
    #test
    if test_fl:
        nn_idx = same_label_topology(net_1,seg_label, k)
    ...
    #test
    if test_fl:
        nn_idx = same_label_topology(net_2,seg_label, k)
...
def get_model_adv_edges(point_cloud, knn, seg_label, input_label, is_training, cat_num, part_num, \
              batch_size, num_point, weight_decay, bn_decay=None):
    batch_size = point_cloud.get_shape()[0].value
    num_point = point_cloud.get_shape()[1].value
    input_image = tf.expand_dims(point_cloud, -1)
