...
def IoU(pred,gt,K):
    ...
    # N = pred.shape[1]
    #
    # gt_classes = np.zeros(shape=[B,K],dtype=np.float32)
    # positive_classes = np.zeros(shape=[B,K],dtype=np.float32)
    # true_positive_classes = np.zeros(shape=[B,K],dtype=np.float32)
    #
    # iou = []
    #
    # for b in range(B):
    ...
    #         pred_l = int(pred[b,i])
    #         gt_classes[b,gt_l] += 1
    #         positive_classes[b,pred_l] += 1
    #         true_positive_classes[b,gt_l] += int(gt_l==pred_l)
    #
    #     iou.append(true_positive_classes[b] / (1e-6 + gt_classes[b] + positive_classes[b] - true_positive_classes[b]))
    #
